...
void drawBlades() {
    glColor3f(0.2,0.1,0.0f); // Blue color for blades
    glBegin(GL_QUADS);
...
void drawHub() {
    glColor3f(1.0f, 0.6f, 0.6f); // Gray color for the hub
    float radius = 0.1f;
    int numSegments = 50;
...
void display() {
    ...
    glLoadIdentity();
    gluLookAt(eyeX, eyeY, eyeZ,  refX, refY, refZ,  0, 1, 0); // Adjust eyeX, eyeY, eyeZ, refX, refY, refZ as needed
    
    // Enable lighting and draw the scene with lighting effects
    ...
    lightBulb2();
    //lightBulb3();
    glDisable(GL_LIGHTING);// Disable lighting for the fan and blades
    // Draw rotating fan and blades
    glPushMatrix();    
    glTranslatef(2.0f, 4.0f, 7.0f); // Move the fan back
    static float angle = 0.0f;
    angle += 0.1f;
    glRotatef(angle, 0.0f, 0.0f, 1.0f); // Rotate the blades

    drawHub(); // Draw the hub of the fan
    drawBlades(); // Draw the blades of the fan
    glPopMatrix();
